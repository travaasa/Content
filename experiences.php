<?php
$xml = simplexml_load_file('xml/copydeck-experiences.xml');
if ($_GET['pageId']){
	$pageId = $_GET['pageId'];
} else {
	$pageId = 0;
...
$count = 0;
$html = "";
foreach ($xml->experiences->secondaryPages->page as $section) {
...
foreach ($xml->experiences->secondaryPages->page as $section) {
	...
	if ($count != 0){
		$html .= '<li><a href="/experiences/'.$section->meta->slug.'" id="'.$section[id].'"' .$selected.' rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
	} else {
		$html .= '<li class="top"><a href="/experiences/'.$section->meta->slug.'" id="'.$section[id].'"'.$selected.' rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';	
	}
	$count++;
...
$imageArray = "";
$countPfd = 0;
$thumbNailArray = "'/images/nav/icon_selected-arrow.png',";
//$pdfDownload = array();
foreach ($xml->experiences->secondaryPages->page as $page) {
	...
	if (($_GET['page'] && $page->meta->slug ==  $page_slug) || (!$_GET['page'] && $page[id] ==  $pageId)){
		...
		$copy = $page->copy;
                $fblink = $page->fblink;
		$content = '<h1>'.$header.'</h1>'.$copy.$fblink;
...
<html>
	...
	<script type="text/javascript" src="/js/jquery.address-1.3.1.min.js?autoUpdate=1&crawling=0&history=1&strict=0"></script>
			...
			if (window.location.pathname != "/experiences/"){
					if(!window.location.hash){
							split_path = window.location.pathname.split('/');
					  		window.location.href = 'http://'+window.location.host+'/experiences/#/'+split_path[2];
						}
			}
		}
...
<!--						<li class="nav0"><a href="/home">Home</a></li>-->
						...
						<li class="nav3"><a class="selected" href="/experiences">Experiences</a></li>
