<?php
$xml = simplexml_load_file('xml/copydeck-giftcards.xml');
if ($_GET['pageId']){
	$pageId = $_GET['pageId'];
} else {
	$pageId = 2;
...
$count = 0;
$html = "";
foreach ($xml->giftcards->secondaryPages->page as $section) {
...
foreach ($xml->giftcards->secondaryPages->page as $section) {
	if ($section[secondaryNav] == "true"){
		...
		if ($count != 0){
			$html .= '<li class="'.$parentNav.$childNav.'"><a href="/gift-cards/'.$section->meta->slug.'" id="'.$section[id].'" class="' .$selected .'" rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
		} else {
			$html .= '<li class="top"><a href="/giftcards/'.$section->meta->slug.'" id="'.$section[id].'" class="'.$selected.'" rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
		}
		$count++;
...
$countPfd = 0;
//$pdfDownload = array();
foreach ($xml->giftcards->secondaryPages->page as $page) {
	...
	<script type="text/javascript">
	  window.onload = function() {
	    transport = new easyXDM.Socket({
	      remote: 'https://www.privatelabelcard.com/iframe.cfm?siteid=wwwtravaasacom2',
	      container: document.getElementById('giftCards'),
	      onMessage: function(message, origin) {
	        if (!isNaN(message) && parseInt(message) >= 160) {
			...
			<div class="content_wrapper">
				...
				<div id="main_nav">
					<ul>
						...
						<li class="nav1"><a href="/about-us/">About Us</a></li>
						<li class="nav2"><a href="/experiences/">Experiences</a></li>
						<li class="nav3"><a href="/austin/">Austin</a></li>
						<li class="nav4"><a href="/hana/">Hāna</a></li>
